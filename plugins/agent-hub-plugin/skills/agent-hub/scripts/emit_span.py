...
リソース属性 (issue #26: bridges#96 と同パターン):
  - ``service.name``                    : f"@{handle}" (AGENT_HUB_PARTICIPANT 環境変数から取得、deprecated alias の AGENT_HUB_USER も fallback; 未設定時は "agent-hub-plugin")
...
def emit_span(msg_id: str, model: str, telemetry_url: str, service_name: str) -> None:
